            ...
            <div class="col blog-main">
                ...
                <div class="pt-md-3 border-top">
                    ...
                    <div class="blog-post text-center">
                        ...
                        $sql = "select count(*) cnt from game";//board의 의 모든 레코드의 숫자를 반환해라.
                        $result = $conn->query($sql);
                        $row = $result->fetch_assoc();
                        $rowcnt = $row['cnt'];
                        ...
                        $sql = "select * from game where title like '$keyword' order by no desc limit $offset, $list_size";
                        $result = $conn->query($sql);
                        if($result->num_rows > 0) {
                        ?>
                        </div>
                                                     ...
                                                     <input type="text" name="no" value="<?=$row['no']?>" hidden>
                                                      <input type="text" name="title" value="<?=$row['title']?>" hidden>
                                                       ...
                                                       <input type="text" name="url" value="<?=$row['targeturl']?>" hidden>
                                                        ...
                                                        <button type="button" onclick="location.href='gamedel.php?no=<?=$row['no']?>'"><span class="fas fa-trash-alt"></span></button>
                                                    </div>
                                                </li>
                                                <?php } ?>
                                            </ul>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    //현재 페이지 이전으로 가도록 prev링크를 만듬.
                                                        if($page > 1)
                                                        { ?>
                                                        <a href="gameshow.php?page=<?=($page-1)?>&keyword=<?=$keyword?>">PREV</a>
                                                        <?php } else { ?>
                                                        <span>PREV</span>
                                                        <?php } ?>
                                                        <?php
                                                        ...
                                                        ?>
                                                        <a href="gameshow.php?page=<?=$p?>&keyword=<?=$keyword?>"><?=$p?></a>
                                                        <?php }} ?>
                                                        <?php
                                                        //현재 페이지 다음에 next 표시하기
                                                        if($page < $end_page)
                                                        { ?>
                                                        <a href="gameshow.php?page=<?=($page+1)?>&keyword=<?=$keyword?>">NEXT</a>
                                                        <?php } else { ?>
                                                        <span>NEXT</span>
                                                        <?php } ?>
                                                    </div>
