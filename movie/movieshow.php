...
<head>
    ...
    <title>VR Community : GameInfo List</title>
        ...
        <div class="row">
            <div class="col blog-main">
                <div class="pb-4 pt-md-5 mb-4 text-center">
                    <h2 class="font-italic">MANAGEMENT</h2>
                    <p>Management gameinfo post... delete or modify</p>
                </div>
                <div class="pt-md-3 border-top">
                    <h3 class="pb-4">
                        Video Post List Management
                    </h3>
                    <div class="blog-post text-center">
                        ...
                        //예외처리 관리자가 아니거나 로그인상태가 아닌 접속을 차단
                        //header("location: ../user/signin.html");
                        }
                        include_once('../dbconn.php');
                        ...
                        $sql = "select count(*) cnt from video";//board의 의 모든 레코드의 숫자를 반환해라.
                        $result = $conn->query($sql);
                        $row = $result->fetch_assoc();
                        $rowcnt = $row['cnt'];
                        ...
                        $sql = "select * from video where title like '$keyword' order by no desc limit $offset, $list_size";
                        $result = $conn->query($sql);
                        if($result->num_rows > 0) {
                        ?>
                        </div>
                                ...
                                <div class="col-md">
                                    <div class="panel panel-primary">
                                        ...
                                        <div class="panel-body">
                                           <form action="moviemod_f.php" method="post">
                                            <ul class="list-group">
                                                     ...
                                                     <input type="text" name="type" value="<?=$row['type']?>" hidden>
                                                      <input type="text" name="title" value="<?=$row['title']?>" hidden>
                                                       ...
                                                       <input type="text" name="url" value="<?=$row['targeturl']?>" hidden>
                                                        ...
                                                        <button type="button" onclick="location.href='moviedel.php?no=<?=$row['no']?>'"><span class="fas fa-trash-alt"></span></button>
                                                    </div>
                                                </li>
                                                <?php } ?>
                                            </ul>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    //현재 페이지 이전으로 가도록 prev링크를 만듬.
                                                        if($page > 1)
                                                        { ?>
                                                        <a href="movieshow.php?page=<?=($page-1)?>&keyword=<?=$keyword?>">PREV</a>
                                                        <?php } else { ?>
                                                        <span>PREV</span>
                                                        <?php } ?>
                                                        <?php
                                                        ...
                                                        ?>
                                                        <a href="movieshow.php?page=<?=$p?>&keyword=<?=$keyword?>"><?=$p?></a>
                                                        <?php }} ?>
                                                        <?php
                                                        //현재 페이지 다음에 next 표시하기
                                                        if($page < $end_page)
                                                        { ?>
                                                        <a href="movieshow.php?page=<?=($page+1)?>&keyword=<?=$keyword?>">NEXT</a>
                                                        <?php } else { ?>
                                                        <span>NEXT</span>
                                                        <?php } ?>
                                                    </div>
