...
if(!isset($_POST['no'])||!isset($_POST['title'])||!isset($_POST['note'])||!isset($_POST['url']))
{
...
if(move_uploaded_file( $_FILES["myfile"]["tmp_name"], "$uploads_dir/$name"))
{
    ...
    $sql = "update video set type = '$type', title = '$title',note='$note',udate=now(),filename ='$name',url ='$url',size =$size,targeturl='$targeturl' where no = $no";
